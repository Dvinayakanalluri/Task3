...
INSERT INTO `Ord Details` Values (10367, 34, 11.2, 36, 0.0);
...
SELECT c.CustomerID, c.CompanyName, 
       COALESCE(SUM(od.Quantity * od.UnitPrice * (1 - od.Discount)), 0) AS TotalRevenue
FROM Customers c
LEFT JOIN Orders o ON c.CustomerID = o.CustomerID
...
FROM (
    SELECT o.OrderID, SUM(od.Quantity * od.UnitPrice * (1 - od.Discount)) AS OrderTotal
    FROM Orders o
...
-- Example usage
SELECT * FROM CustomerTotalSales ORDER BY TotalSales DESC;
